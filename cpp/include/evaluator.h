...
protected:
    ...
    class KillPath {
    public:
        KillPath(ChessPath path, int steps) : path(path), steps(steps) {}
...
public:
    ...
    /**
     * 随机程度。
     * 随机出棋：假定当前评估越大越好。假设最大价值是 v_max,
     * 则随机选择路径价值在 [v_max * rand_state, v_max] 作为最终选择。
     */
    float rand_state = 0.5;
