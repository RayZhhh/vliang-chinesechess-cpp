...
void calculate_each_path(Chessboard board, ChessPath &path, int depth, TreeType type, bool print_porcess) {
...
ChessPath MultiThreadEvaluator::get_best_path() {
    ...
    for (ChessPath &path: paths) {
        threads.emplace_back(thread(calculate_each_path, this->board, std::ref(path), this->depth, this->tree_type, this->print_res));
    }
