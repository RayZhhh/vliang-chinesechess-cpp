...
//    }
//
//    void parse_random(const string &rand) {
//        float rand_state = std::atof(rand.c_str());
...
//        cerr << "argv[1]: the sequence of the chessboard." << endl;
//        cerr << "argv[2]: the type of the searching tree (or searching algorithm)." << endl;
//        cerr << "argv[3]: the search depth." << endl;
//        cerr << "argv[4]: the target chess color." << endl;
//        cerr << "argv[5]: the path of the file which stores the evaluate results." << endl;
//        cerr << "argv[6]: if get random result, default = -1. If argv[6] != -1, random_state = argv[6]." << endl;
//        return;
//    }
//
//    ArgParser parser;
...
//    if (argv[6]) {
//        parser.parse_random((argv[6]));
////        cout << parser.rand_state << endl;
//    }
//
//    MultiThreadEvaluator evaluator(parser.board, parser.depth, parser.tree_type, parser.color_sign);
//    auto ret_path = parser.rand_state == -1 ? evaluator.get_best_path() : evaluator.get_rand_path();
//    evaluator.print_res = false;
//
//    string path = argv[5];
//    ofstream out(path, ios::out);
//    out << ret_path.from_x << "," << ret_path.from_y << "," << ret_path.to_x << "," << ret_path.to_y << ","
//        << ret_path.eat;
//}
//
//
